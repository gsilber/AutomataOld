...
export class FsmExamplesComponent implements OnInit {
  @Output() exampleload: EventEmitter<any> = new EventEmitter<any>();
  examples = [
    {
      sigma: '{a,b}',
    ...
    {
      sigma: '{a,b}',
      description: 'The union of two deterministic FSM with Epsilon transitions.  ' +
        'Accepts strings that start and end with at least two a\'s',
      // tslint:disable-next-line:max-line-length
  ...
  loadMachine(machDescription) {
    this.exampleload.emit(machDescription.data);
  }
